...
class Keyboard extends React.Component<KeyboardProps> {
  ...
  getNotePointsOffset(note: Note) {
    ...
    const data = noteToPoints[rawNote];
  ...
  render() {
    ...
    const paths = notes.map( (obj, idx) => {
      ...
      const ret = (
        <path key={idx} d={utility.toPath(obj.points, offset)} fill={blackNote ? black : white} />
      );
      offset += obj.offset;
      return ret;
    });
    ...
    return (
      <svg width="400" height="300" viewBox="0 0 170 60" xmlns="http://www.w3.org/2000/svg">
        {paths}
      </svg>
    );
  }
